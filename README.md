...
- 바라보는 방향 우선 로드
- 최근 언로드 청크의 데이터 캐시
- 스트리밍 상태와 적용 책임 분리 (`ChunkStreamer` / `ChunkManager`)
...
- [/Users/wmk/Desktop/Projects/VoxelEngine/Assets/Scripts/UnityBridge/Streaming/ChunkStreamer.cs](/Users/wmk/Desktop/Projects/VoxelEngine/Assets/Scripts/UnityBridge/Streaming/ChunkStreamer.cs)
...
- [/Users/wmk/Desktop/Projects/VoxelEngine/Assets/Scripts/UnityBridge/Settings/TerrainGenerationSettings.cs](/Users/wmk/Desktop/Projects/VoxelEngine/Assets/Scripts/UnityBridge/Settings/TerrainGenerationSettings.cs)
...
- **스트리밍 상태 분리**  
  required/unload 집합 캐시, unload delta 계산, load plan 생성을 `ChunkStreamer`로 분리해 `ChunkManager`의 책임을 줄였습니다.
...
- **콜라이더 갱신 책임 분리**  
  collider on/off 갱신과 디버그 기즈모 처리를 `ChunkColliderUpdater`로 분리해 스트리밍 로직과 관심사를 나눴습니다.
...
- 스트리밍 오케스트레이션
- 스트리밍 상태 캐시와 load/unload delta 계산
- 청크 GameObject 관리
